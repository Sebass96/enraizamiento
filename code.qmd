...
desarrollo$peso_raices[desarrollo$peso_raices == 0] <- NA
...
fig <- enraizamiento %>% 
  ...
  scale_fill_grey(labels = c("Callused","Rooted","Dead"), start = 0.8, end = 0.2) +
...
bartlett.test(numero_raices ~ tratamiento, desarrollo)

# Test de Levene

leveneTest(numero_raices ~ tratamiento, desarrollo)
...
mc <- emmeans(model, ~ tratamiento) %>%
  cld(Letters = letters, reversed = F) %>%
  mutate(across(.group, trimws)) %>% 
  rename(group = ".group")

tratamiento
SNK.test(mode,"tratamiento", console = T)
...
bartlett.test(longitud_raices ~ tratamiento, desarrollo)

# Test de Levene

leveneTest(longitud_raices ~ tratamiento, desarrollo)
...
SNK.test(model,"tratamiento", console = T)
duncan.test(model, "tratamiento", group=F)$comparison
LSD.test(y=model, trt = "tratamiento",group = T, console = T)
...
bartlett.test(peso_raices ~ tratamiento, desarrollo)

# Test de Levene

leveneTest(peso_raices ~ tratamiento, desarrollo)
...
kruskal.test(peso_raices ~ tratamiento, data = desarrollo)
